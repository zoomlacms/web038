...
public class wxapp : API_Base, IHttpHandler
{
    ...
    public string siteUrl = "https://a000.2018.z01.com";
    public int AppId {get { return DataConvert.CLng(Req("appId"),2); } }
    //---------------Extend
        ...
        try
        {
            switch (Action)
            {
                ...
                case "content_list":
                    {
                        ...
                        {
                            DataRow dr = setting.dt.Rows[i];
                            dr["TopImg"] = H_ImageUrl(DataConvert.CStr(dr["TopImg"]));
                        }
                        retMod.retcode = M_APIResult.Success;
                        retMod.result = JsonConvert.SerializeObject(setting.dt);
                        retMod.page = new M_API_Page(setting);
                ...
                case "payment_cart"://购物车生成订单后支付
                    {
                        WXAPPAPI api = WXAPPAPI.Code_Get(2);
